...
void add_to_tree(int x, int previousx)
//x - current vertex,previousx - vertex from X before x in the alternating path,
//so we add edges (previousx, xy[x]), (xy[x], x)
{
    S[x] = true; //add x to S
    previous[x] = previousx; //we need this when augmenting
